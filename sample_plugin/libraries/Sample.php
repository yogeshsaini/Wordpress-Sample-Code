...
/**
 * Provides access to the Sample API.
 *
...
class Sample
{
     ...
     * on Sample. Once authorized, a refresh token is requested by the API client to the token
     ...
     * The API enpoint URL
     */
    public $apiEndpointUrl = 'https://api.sample.com';

    /**
     * The OAuth authorization server endpoint URL
     */
    public $oauthAuthorizeEndpointUrl = 'https://api.sample.com/oauth/authorize';

    /**
     * The OAuth token server enpoind URL
     */
    public $oauthTokenEndpointUrl = 'https://api.sample.com/oauth/token';
     ...
     *
     * To create an API key/secret pair, go to: http://www.sample.com/profile/developer
     *
     * @param $type Integer can be one of Sample::GRANT_TYPE_AUTHORIZATION, Sample::GRANT_TYPE_CLIENT_CREDENTIALS
     *                      or Sample::GRANT_TYPE_PASSWORD.
     * @param $apiKey the API key
     * @param $apiSecret the API secret
     * @param $scope mixed the permission scope requested (see http://www.sample.com/doc/api/authentication.html#requesting-extended-permissions
     ...
     *
     * Info Keys:
     * - redirect_uri: if $type is Sample::GRANT_TYPE_AUTHORIZATION, this key can be provided. If omited,
     ...
     *
     * @param $scope Array a list of requested scope (see http://www.sample.com/doc/api/authentication.html#requesting-extended-permissions
     *                     for a list of available permissions)
    ...
    /**
     * Upload a file on the Sample servers and generate an URL to be used with API methods.
     *
    ...
    public function uploadFile($filePath)
    {
        ...
        $this->timeout = null;
        $result        = json_decode($this->httpRequest($result['upload_url'], array(
            'file' => '@' . $filePath
        )), true);
    ...
    public function call($method, $args = array())
    {
        ...
        if (!isset($result)) {
            throw new SampleApiException('Invalid API server response.');
        } elseif ($status_code !== 200) {
            throw new SampleApiException('Unknown error: ' . $status_code, $status_code);
            ...
            throw new SampleApiException("Invalid API server response: no 'result' key found.");
        }
            ...
            if ($this->grantType === self::GRANT_TYPE_AUTHORIZATION) {
                if (isset($_GET['code'])) {
                    ...
                    if ($_GET['error'] === 'access_denied') {
                        $e = new SampleAuthRefusedException($message);
                    } else {
                        $e = new SampleAuthException($message);
                    }
                    $e->error = $_GET['error'];
                    throw $e;
                } else {
    ...
    {
        global $current_user;
        $optionvalue = get_user_meta($current_user->ID, 'sample_session_store', false);
        if (!empty($optionvalue[0])) {
            $session = $optionvalue[0];
            return $session;
        }
    ...
    {
        global $current_user;
        $dm_session_meta = get_user_meta($current_user->ID, 'sample_session_store', false);
        $new_value   = $session;
        if(!empty($new_value)) {
     ...
     *
     * @throws SampleAuthException in case of token server error or invalid response
     */
    protected function oauthTokenRequest(Array $args)
    {
        ...
        if (!isset($result)) {
            throw new SampleAuthException("Invalid token server response: $response.");
            ...
            throw new SampleAuthException('No access token found in the token server response.');
        }
    }
     ...
     *
     * @throws SampleAuthException if an oauth error occurs
     * @throws SampleTransportException if an error occurs during request.
     */
    ...
    protected function httpRequest($url, $payload, $headers = null, &$status_code = null, &$response_headers = null, $encode_payload = false)
    {
        ...
        @curl_setopt_array($ch, array(
            ...
            CURLOPT_USERAGENT => sprintf('Sample-PHP/%s (PHP %s; %s)', self::VERSION, PHP_VERSION, php_sapi_name()),
            CURLOPT_HEADER => true,
        ...
        if ($response === false) {
            $e = new SampleTransportException(curl_error($ch), curl_errno($ch));
            curl_close($ch);
            throw $e;
        }
    ...
    protected function getCurrentUrl()
    {
        ...
        $currentUrl = $scheme . $_SERVER['HTTP_HOST'] . str_replace('admin-ajax.php', 'admin.php?page=dm-admin-setting', $_SERVER['REQUEST_URI']);
        ...
        if (@$parts['query'] !== '') {
            ...
            foreach (array('code', 'scope', 'error', 'error_description', 'error_uri', 'state', 'uid', 'sig') as $name) {
                unset($params[$name]);
            }
            if (count($params) > 0) {
...
class SampleApiException extends Exception
{
}

class SampleTransportException extends SampleApiException
{
}

class SampleAuthException extends SampleApiException
{
    public $error = null;
}
...
class SampleAuthRefusedException extends SampleAuthException
{
}