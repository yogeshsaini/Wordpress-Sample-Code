...
class MyVideoGalleryPage
{
    ...
    public function __construct($dcuname = null, $dcpass = null)
    {
        ...
        add_action('wp_ajax_delete_dm_cloud_metatags', array(
            $this,
            'deleteDMCloudMetatags'
        ));
        add_action('wp_ajax_update_dm_cloud_metatags', array(
            $this,
            'updateDMCloudMetatags'
        ));
        //add_action('admin_menu', array(
        //    $this,
        //    'sampleCloudVideoDetailPage'
        //));
        add_action('wp_ajax_action_upload_pattern', array(
            $this,
            'sampleCloudUploadPattern'
        ));
        add_action('wp_ajax_samplecloud_change_gallery_records', array(
            $this,
            'getSampleCloudVideos'
        ));
    }

    /**
     * Callback for Sample cloud video detail page
     *
     */
    //public function sampleCloudVideoDetailPage()
    //{
    //    add_submenu_page(null, 'Sample cloud video', 'Sample cloud video', 'administrator', 'dmcloud-video-detail', array(
    //        $this,
    //        'sampleCloudVideoDetailPageCallback'
    //    ));
    //}

    /**
     * Callback method for displaying Sample cloud video gallery page
     *
     */
    //public function sampleCloudVideoDetailPageCallback()
    //{
    //    require_once SAMPLE_DIR . '/dmc/dmcloud-video-detail.php';
    //}
     ...
     *
     */
    public function getSampleCloudVideos($search_title = '', $sortby = '-created')
    {
        $samplecloud      = new SampleCloudOwnMethod($this->dcuname, $this->dcpass);
        $pn                    = (isset($_GET['pagenum'])) ? preg_replace('#[^0-9]#i', '', $_GET['pagenum']) : 1;
        $itemsPerPage          = (int) self::ITEMS_PER_PAGE;
        $return_dmcloud_videos = $samplecloud->getSampleCloudVideos((int) $pn, $itemsPerPage, $search_title, $sortby);
     ...
     *
     */
    public function getDMCKeywords($mediaId)
    {
        $samplecloud = new SampleCloudOwnMethod($this->dcuname, $this->dcpass);
        return $samplecloud->getSampleCloudVideosKeywords($mediaId);
    }
     ...
     *
     */
    public function deleteDMCloudRecordsCallback()
    {
        $media_id = ($_POST['mediaId']) ? $_POST['mediaId'] : null;
        if (!empty($media_id)) {
            $samplecloud = new SampleCloudOwnMethod($this->dcuname, $this->dcpass);
            $delete           = ($samplecloud->deleteSampleCloudMedia($media_id)) ? true : false;
            session_start();
	...
	if (!empty($media_id)) {
	    $samplecloud = new SampleCloudOwnMethod($this->dcuname, $this->dcpass);
	    $videoInfo        = $samplecloud->getSampleCloudVideosDetails($media_id);
            $playerList = $samplecloud->getSampleCloudPlayer();
    ...
    public function updateDMCloudMetatags()
    {
        ...
        if ($mediaId) {
            ...
            if ($imageurl) {
                $samplecloud->setSampleCloudVideoThumbnail($mediaId, $imageurl);
            }
            if ($updatemeta) {
                $samplecloud->removeSampleCloudVideoMetas($mediaId, $updatemeta);
                $samplecloud->setSampleCloudVideoMetas($mediaId, $originalmetameta);
            }
            if ($meta) {
                $samplecloud->setSampleCloudVideoMetas($mediaId, $meta);
            }
            session_start();
            if(!isset($_SESSION['dmc_success']) || $_SESSION['dmc_success'] == "")
            {
     ...
     *
     */
    public function sampleCloudUploadPattern()
    {
